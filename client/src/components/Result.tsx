...
import { ChevronDown, BookOpen, Users, Trophy, Download } from "lucide-react";
...
interface ExtractedData {
  [deptCode: string]: DepartmentData;
}
...
const Result = () => {
  ...
  const isValidResultsData = resultsData && typeof resultsData === 'object' && Object.keys(resultsData).length > 0;

  const handleDepartmentChange = (deptCode: string) => {
    if (!resultsData[deptCode]) {
      setError(`Invalid department code: ${deptCode}`);
      return;
    }
  ...
  const getGradeColor = (grade: string) => {
    switch (grade) {
      ...
      case "A+": case "A": return "bg-blue-100 text-blue-800 border-blue-200";
      case "B+": case "B": return "bg-indigo-100 text-indigo-800 border-indigo-200";
      case "C+": case "C": return "bg-purple-100 text-purple-800 border-purple-200";
      ...
      case "Withheld": return "bg-gray-200 text-gray-700 border-gray-300";
  ...
  const filterStudents = (students: StudentResult[]) => {
    ...
    // Apply range filter
    if (studentRange) {
      const [startReg, endReg] = studentRange.split("-").map(s => s.trim());
      if (startReg && endReg) {
    ...
    if (filterBy === "none") return filtered;
    if (filterBy === "allPass") {
      return filtered.filter((student) =>
        Object.values(student.courses).every((grade) => !failingGrades.includes(grade))
      );
    }
  ...
  const departmentStats = selectedDepartment && resultsData[selectedDepartment]
    ? (() => {
        ...
        return {
          totalStudents,
          passPercentage: totalStudents > 0 ? Number(((passed / totalStudents) * 100).toFixed(2)) : 0,
          totalCourses: Object.keys(resultsData[selectedDepartment].courses || {}).length,
        };
      })()
    : null;

  const courseStats = selectedDepartment && resultsData[selectedDepartment]
    ? (() => {
        const students = resultsData[selectedDepartment].students || [];
        const totalStudents = students.length;
  ...
  const generateReportContent = () => {
    if (!selectedDepartment || selectedStudents.size === 0 || !resultsData[selectedDepartment]) return "";

    const dept = resultsData[selectedDepartment];
    const filteredStudents = sortStudents(filterStudents(dept.students)).filter(
      (s) => selectedStudents.has(s.registerNo)
    );
    ...
    content += `Class & Sem                                               : ${classSem || "N/A"}\n`;
    ...
    content += `Total Number of students failed in all subjects\t\t${
      filteredStudents.filter((s) => countFails(s) === Object.keys(dept.courses).length).length
    }\n\n`;
  ...
  const generateStudentListContent = () => {
    if (!selectedDepartment || selectedStudents.size === 0 || !resultsData[selectedDepartment]) {
      return { rows: [], excelColWidths: [] };
    }
    ...
    const rows: string[][] = [];
    rows.push(["MANGALAM COLLEGE OF ENGINEERING"]);
    rows.push([`DEPARTMENT OF ${dept.name.toUpperCase()}`]);
    rows.push([`UNIVERSITY RESULT ANALYSIS ${new Date().toLocaleString("default", { month: "long" }).toUpperCase()} 2025`]);
    rows.push([]);
    rows.push(["Class & Sem", classSem || "N/A"]);
    rows.push(["Academic Semester", "JAN 2025 - MAY 2025"]);
    rows.push(["Total number of Students registered", totalSelected.toString()]);
    rows.push(["Total number of Students all cleared", passedSelected.toString()]);
    rows.push(["Pass Percentage in S2", `(${passedSelected}/${totalSelected}) ${passPercentage}%`]);
    rows.push(["Overall Pass % upto S2", `(${passedSelected}/${totalSelected}) ${passPercentage}%`]);
    rows.push([]);

    const studentHeader = ["Sl. No.", "Reg No", "Name of Student", ...courseCodes.map(() => "Subject Code"), "No.of supplies in S2"];
    rows.push(studentHeader);
    rows.push(["", "", "", ...courseCodes, ""]);

    filteredStudents.forEach((student, index) => {
      const slNo = (index + 1).toString();
      const grades = courseCodes.map((code) => student.courses[code] || "N/A");
      rows.push([slNo, student.registerNo, student.name || "N/A", ...grades, countFails(student).toString()]);
    });

    rows.push([]);
    rows.push(["No.of Pass", "", "", ...courseCodes.map((code) => passCounts[code].toString()), ""]);
    rows.push(["No.of Failures", "", "", ...courseCodes.map((code) => failureCounts[code].toString()), ""]);
    rows.push(["No.of Withheld", "", "", ...courseCodes.map((code) => withheldCounts[code].toString()), ""]);
    rows.push(["No.of Absentees", "", "", ...courseCodes.map((code) => absentCounts[code].toString()), ""]);
    rows.push(["Pass Percentage", "", "", ...courseCodes.map((code) => passPercentages[code].toFixed(2)), ""]);
    rows.push([]);

    rows.push(["Sl No", "Subject Code", "Subject Name", "Subject Handled by", "% of pass"]);
    let slNo = 1;
    Object.entries(dept.courses).forEach(([code, name]) => {
      const teacher = teachers[code] || "Staff Name";
      const percentage = passPercentages[code].toFixed(2);
      rows.push([slNo.toString(), code, name, teacher, percentage]);
      slNo++;
    });

    rows.push([]);
    rows.push(["CLASS IN CHARGE", "", "HOD", "", "PRINCIPAL"]);

    const maxWidths = new Array(Math.max(...rows.map((r) => r.length))).fill(0);
    rows.forEach((row) => {
      row.forEach((cell, colIdx) => {
        if (cell) {
          const cellWidth = cell.toString().length;
          maxWidths[colIdx] = Math.max(maxWidths[colIdx], cellWidth);
        }
      });
    });

    const excelColWidths = maxWidths.map((width, idx) => {
      const header = rows.find((row) => row[idx] && row[idx].toString().length > 0)?.[idx] || "";
      const minWidth = header.toString().length + 2;
  ...
  const exportToExcel = () => {
    ...
    if (downloadType === "summary") {
      ...
      const content = generateStudentListContent();
      const ws = XLSX.utils.aoa_to_sheet(content.rows);
      ws["!cols"] = content.excelColWidths;
      XLSX.utils.book_append_sheet(wb, ws, "ResultAnalysis");
    }
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(
      `DEPARTMENT OF ${resultsData[selectedDepartment]?.name?.toUpperCase() || "N/A"}`,
      pageWidth / 2,
      17,
      { align: "center" }
    );
  ...
  if (!isValidResultsData) {
    return (
      <div className="text-center px-10 py-12">
        <BookOpen className="h-16 w-16 text-gray-300 mx-auto mb-4" />
        <h3 className="text-xl font-medium text-gray-900 mb-2">No Data Available</h3>
        <p className="text-gray-600">Please upload a PDF to extract results or check if the data format is correct.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      {error && (
        <div className="px-10 my-4 text-red-600 bg-red-100 p-4 rounded-lg">
          {error}
        </div>
      )}
      {/* Department Selection, Sorting, and Filtering */}
      <div className="mb-8 grid px-10 mx-10 mt-4 grid-cols-1 md:grid-cols-4 gap-4">
        <div className="relative">
          ...
          >
            <div className="flex items-center justify-between">
              <span className={selectedDepartment ? "text-gray-900" : "text-gray-500"}>
                {selectedDepartment ? `${resultsData[selectedDepartment].name} (${selectedDepartment})` : "Choose a department..."}
              </span>
              <ChevronDown
          ...
          {isDropdownOpen && (
            <div className="absolute z-10 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-auto">
              {Object.entries(resultsData as ExtractedData).map(([code, dept]) => (
                <button
                  key={code}
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">Department Code: {code}</div>
                </button>
              ))}
            </div>
          )}
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-gray-900">
                {sortOption === "allPass" ? "All Pass (No Failures)" : "Register Number"}
              </span>
              <ChevronDown
                className={`h-5 w-5 text-gray-400 transition-transform duration-200 ${isSortDropdownOpen ? "rotate-180" : ""}`}
              />
            </div>
          </button>
          {isSortDropdownOpen && (
            <div className="absolute z-10 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-auto">
              <button
                ...
                className="w-full text-left px-4 py-3 hover:bg-blue-50 focus:bg-blue-50 focus:outline-none transition-colors duration-150 border-b border-gray-100 last:border-b-0"
              >
                <div className="font-medium text-gray-900">Register Number</div>
              </button>
              <button
                onClick={() => handleSortChange("allPass")}
                className="w-full text-left px-4 py-3 hover:bg-blue-50 focus:bg-blue-50 focus:outline-none transition-colors duration-150 border-b border-gray-100 last:border-b-0"
              >
                <div className="font-medium text-gray-900">All Pass (No Failures)</div>
              </button>
            </div>
          )}
        </div>
        <div className="relative">
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-gray-900">{getFilterDisplay()}</span>
              <ChevronDown
                className={`h-5 w-5 text-gray-400 transition-transform duration-200 ${isFilterDropdownOpen ? "rotate-180" : ""}`}
              />
            </div>
          </button>
        ...
        <div className="relative">
          <label className="block text-sm font-medium text-gray-700 mb-2">Filter by Register Range</label>
          <input
            type="text"
            value={studentRange}
      ...
      {selectedDepartment && (
        <div className="my-4 px-10 flex gap-4">
          <button
      ...
      {downloadFormVisible && (
        <div className="fixed inset-0 px-10 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      ...
      {selectedDepartment && (
        <div className="grid px-10 grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-blue-500">
            <div className="flex items-center">
              <Users className="h-8 w-8 text-blue-500 mr-3" />
              <div>
                <p className="text-sm font-medium text-gray-600">Total Students</p>
                <p className="text-2xl font-bold text-gray-900">{departmentStats?.totalStudents || 0}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-green-500">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{departmentStats?.passPercentage || 0}%</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-purple-500">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{departmentStats?.totalCourses || 0}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {selectedDepartment && (
        <div className="bg-white px-10 rounded-xl shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-gray-50 to-blue-50 px-6 py-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">Academic Year: 2024-25 | Semester: 2</p>
          </div>
          ...
          <div className="relative max-h-[600px] overflow-y-auto">
            <table className="w-full">
                ...
                <tr>
                  <th className="sticky left-0 top-0 bg-gray-50 px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200 z-20">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked) {
                          sortStudents(filterStudents(resultsData[selectedDepartment].students)).forEach((s) => newSelected.add(s.registerNo));
                        }
                  ...
                  {Object.entries(resultsData[selectedDepartment].courses).map(([code, name]) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-400 normal-case mt-1 line-clamp-2">{name}</div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortStudents(filterStudents(resultsData[selectedDepartment].students)).map((student, index) => (
                  <tr key={student.registerNo} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    {Object.keys(resultsData[selectedDepartment].courses).map((courseCode) => (
                      <td key={courseCode} className="px-3 py-4 whitespace-nowrap text-center">
                        <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full border ${getGradeColor(student.courses[courseCode] || "N/A")}`}>
